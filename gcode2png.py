...
class GcodeRenderer:
    ...
    def plotModel(self):
        mlab.plot3d(self.coords['object']['x'], self.coords['object']['y'], self.coords['object']['z'],
                    color=self.extrudecolor, line_width=2.0, representation='wireframe')
        if len(self.coords['moves']['x']) > 0:
            mlab.plot3d(self.coords['moves']['x'], self.coords['moves']['y'], self.coords['moves']['z'],
                        color=self.movecolor, line_width=2.0, representation='wireframe')

    def plotSupport(self):
        if len(self.coords['support']['x']) > 0:
            mlab.plot3d(self.coords['support']['x'], self.coords['support']['y'], self.coords['support']['z'],
                        color=self.supportcolor, tube_radius=0.5)
